...
export type TabKey = 'dashboard' | 'pos' | 'repairs' | 'inventory' | 'history' | 'users' | 'profile' | 'stock' | 'my-orders';
...
function NavItem({
  ...
  return (
    <div
      ...
      style={{
        backgroundColor: active ? 'var(--c-primary-soft)' : hovered ? 'var(--c-hover)' : 'transparent',
      }}
...
export function Sidebar({
  ...
  const allTabs: { key: TabKey; label: string; section: 'operations' | 'admin' }[] = [
    ...
    { key: 'my-orders', label: t('sidebar.myOrders'), section: 'admin' },
  ];

  const tabs = allTabs.filter(({ key }) => {
    if (role === 'ADMIN') return true;
    if (role === 'TECH_IT') return key !== 'users';
    if (role === 'TECH_COM') return key === 'my-orders';
    return false;
  });
    ...
    return (
      <div className="flex flex-col gap-0.5">
        <span className={SECTION_HEADER}>
          {t(section === 'operations' ? 'sidebar.sectionOperations' : 'sidebar.sectionAdmin',
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between mb-8 pl-4 pr-2">
        <div className="flex items-center gap-2.5 min-w-0 font-display-md">
          ...
          <div className="min-w-0 tracking-tight text-xs uppercase flex gap-1 items-baseline">
            ...
            <span className="font-normal text-on-surface-variant">TECHNOLOGY</span>
          </div>
        </div>
        <button
        ...
        >
          <MaterialIcon icon={darkMode ? 'light_mode' : 'dark_mode'} wght={300} size={18} />
        </button>
      </div>
      ...
      <div className="flex items-center gap-2 mb-6 pl-4 pr-3">
        <div className="flex flex-col gap-0.5 flex-1 min-w-0">
          ...
          <span className="text-[10px] text-on-surface-variant/60 tracking-widest uppercase font-medium">{user?.role}</span>
        </div>
        <button
        ...
        >
          <MaterialIcon icon="person" wght={300} className="text-on-surface-variant" size={18} />
        </button>
      </div>
  ...
  return (
    <>
      <aside className="hidden lg:flex w-64 py-5 pr-3 flex-col border-r"
        style={{ backgroundColor: 'var(--c-sidebar)', borderColor: 'var(--c-sidebar-border)' }}>
        {nav}
      </aside>

      {/* ── Mobile top bar ── */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-40 glass-panel px-4 py-3 flex items-center justify-between">
        <button onClick={() => setMobileOpen(true)} className="neo-btn p-2 rounded-lg">
          <MaterialIcon icon="menu" wght={300} className="text-on-surface-variant" size={20} />
        </button>
        <div className="flex items-center gap-2 font-display-md">
          <div className="h-8 w-8 rounded-lg bg-cyan-accent flex items-center justify-center text-[#0A0A0A] font-bold text-xs">∞</div>
          <span className="font-bold text-xs uppercase text-on-surface">INFINITY TECHNOLOGY</span>
        </div>
        <button onClick={onToggleTheme} className="neo-btn h-8 w-8 rounded-lg flex items-center justify-center">
          <MaterialIcon icon={darkMode ? 'light_mode' : 'dark_mode'} wght={300} className="text-cyan-accent" size={18} />
        </button>
      </div>
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-72 py-5 pr-3 border-r border-outline-variant overflow-y-auto"
            style={{ backgroundColor: 'var(--c-sidebar)' }}>
            <div className="flex justify-end px-4 mb-2">
              <button onClick={() => setMobileOpen(false)} className="neo-btn p-2 rounded-lg">
                <MaterialIcon icon="close" wght={300} className="text-on-surface-variant" size={20} />
              </button>
            </div>
            {nav}
          </div>
